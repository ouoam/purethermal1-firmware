...
int main(void)
{
  ...
  /* USER CODE BEGIN 3 */
    ...
    lepton_task(&lepton_task_pt);
#ifndef THERMAL_DATA_UART
    usb_task(&usb_task_pt);
#else
    uart_task(&uart_task_pt);
#endif
    if (pt_board == PT_BOARD_PT1) {
      button_task(&button_task_pt);
    }

    lepton_attribute_xfer_task(&lepton_attribute_xfer_task_pt);
...
static void MX_GPIO_Init(void)
{
  ...
  /*Configure GPIO pin Output Level */
  HAL_GPIO_WritePin(GPIOB, BUCK_ON_Pin|LDO_ON_Pin, GPIO_PIN_SET);
