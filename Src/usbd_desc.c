...
#define USBD_PRODUCT_STRING_FS_TMP USBD_PRODUCT_STRING_FS " (fw:" USBD_SERIALNUMBER_STRING_FS ")"
...
uint8_t * USBD_FS_ProductStrDescriptor(USBD_SpeedTypeDef speed, uint16_t *length)
{
  if(speed == 0)
  {
    USBD_GetString((uint8_t *)USBD_PRODUCT_STRING_FS_TMP, USBD_StrDesc, length);
  }
  else
  {
    USBD_GetString((uint8_t *)USBD_PRODUCT_STRING_FS_TMP, USBD_StrDesc, length);
